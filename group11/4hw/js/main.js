...
function calcOptimalWeight() {
	...
	if (female1.checked) {
		if (bodyWeightIndex < 19) {
			document.getElementById("result").innerHTML = 'У вас дефицит массы тела.';
		} else if (bodyWeightIndex >= 19 && bodyWeightIndex <= 24) {
			document.getElementById("result").innerHTML = 'Масса вашего тела в норме.';
		} else if (bodyWeightIndex > 24 && bodyWeightIndex < 25) {
			document.getElementById("result").innerHTML = 'У вас незначительный избыток массы тела.';
		} else {
			document.getElementById("result").innerHTML = 'У вас избыток массы тела.';
		}
	...
	document.getElementById("ind").innerHTML = '-------------------------------------------------------- <br> Индекс массы тела расчитывается по формуле Кетле - масса тела в килограммах, разделённая на возведённый в квадрат рост в метрах. Нормальный показатель индекса Кетле для мужчин - 19-25, для женщин - 19-24. ';
}